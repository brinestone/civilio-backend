...
export const choiceGroups = civilio.table('choice_groups', {
  title: text().notNull(),
  form: text().notNull(),
  ...
  index().on(t.parentKey, t.parentValue),
  index().on(t.key),
  primaryKey({
    columns: [t.form, t.key]
  }),
  foreignKey({
    columns: [t.form, t.parentKey],
    foreignColumns: [t.form, t.key]
  }).onUpdate('cascade').onDelete('set null'),
  foreignKey({
    columns: [t.parentKey, t.form, t.parentValue],
    foreignColumns: [choiceValues.key, choiceValues.form, choiceValues.value]
  }).onUpdate('cascade').onDelete('restrict'),
  foreignKey({
    columns: [t.form],
    foreignColumns: [formDefinitions.name]
  }).onDelete('cascade').onUpdate('cascade')
]);

export const choiceValues = civilio.table('choice_values', {
  label: text().notNull(),
  value: text().notNull(),
  ...
  primaryKey({
    columns: [t.key, t.form, t.value]
  }),
  foreignKey({
    columns: [t.key, t.form],
    foreignColumns: [choiceGroups.key, choiceGroups.form]
  }).onDelete('cascade').onUpdate('cascade')
]);
...
export const choices = civilio.table(
  ...
  (table) => [
    ...
    //     check('parent_check', sql`
    //   (${table.parentValue} IS NULL AND ${table.parentGroup} IS NULL) OR (${table.parentValue} IS NOT NULL AND ${table.parentGroup} IS NOT NULL)
    // `),
    // foreignKey({
    //   columns: [table.parentGroup, table.parentValue],
    //   foreignColumns: [table.parentGroup, table.parentValue]
    // }).onDelete('set null')
    //     .onUpdate('cascade')
    // TODO: enable this when dynamic forms feature is working
    // foreignKey({
...
export const formSections = civilio.table('form_sections', {
  key: text().notNull(),
  form: text().notNull().references(() => formDefinitions.name, {
    onDelete: 'cascade',
    onUpdate: 'cascade'
  }),
  title: text().notNull(),
...
export const formFields = civilio.table('form_fields', {
  ...
  createdAt: timestamp('created_at', { mode: 'date' }).notNull().defaultNow(),
  updatedAt: timestamp('updated_at', { mode: 'date' }).notNull().defaultNow().$onUpdate(() => new Date()),
  form: text('form_name').notNull().references(() => formDefinitions.name, {
    onDelete: 'cascade',
    onUpdate: 'cascade'
  }),
  ...
  index().on(t.title),
  index().on(t.description).where(isNotNull(t.description)),
  index().on(t.titleTranslated).where(isNotNull(t.description)),
  index().on(t.descriptionTranslated).where(isNotNull(t.description)),
  index().on(t.sectionKey),
  foreignKey({
    columns: [t.sectionKey, t.form],
    foreignColumns: [formSections.key, formSections.form]
  }).onDelete('cascade').onUpdate('cascade')
]);
...
export const relations = defineRelations({
  ...
  choiceGroups: {
    options: r.many.choiceValues({
      from: [r.choiceGroups.key, r.choiceGroups.form],
      to: [r.choiceValues.key, r.choiceValues.form],
    }),
    parent: r.one.choiceGroups({
      optional: true,
      from: [r.choiceGroups.parentKey, r.choiceGroups.form],
      to: [r.choiceGroups.key, r.choiceGroups.form]
    })
  },
  formDefinitions: {
    fields: r.many.formFields({
      from: r.formDefinitions.name,
      to: r.formFields.form
    }),
    sections: r.many.formSections({
      from: r.formDefinitions.name,
      to: r.formSections.form
    })
  },
  formSections: {
    fields: r.many.formFields({
      from: r.formSections.key,
      to: r.formFields.sectionKey
    })
  }
}));
