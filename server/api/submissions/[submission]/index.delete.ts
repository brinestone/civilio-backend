...
defineRouteMeta({
	openAPI: {
		...
		parameters: [
			...
			{ in: 'query', name: 'fv', schema: { type: 'string' }, required: false, description: 'The form version to use, leave empty to use the current version' },
			{ in: 'query', name: 'sv', schema: { type: 'string' }, required: false, description: 'The submission version to use, leave empty to delete all versions, use "current" to delete the current version' },
		],
		responses: {
			'202': {
